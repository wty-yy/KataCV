...
def get_args_and_writer():
    ...
    args.run_name = f"{args.model_name}__load_{args.load_weights_id}__lr_{args.learning_rate}__batch{args.batch_size}__{datetime.datetime.now().strftime(r'%Y%m%d_%H%M%S')}".replace("/", "-")
    if args.wandb_track:
        import wandb
        wandb.init(
