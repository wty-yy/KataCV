...
def boxes_to_cells(labels: tf.Tensor, params: tf.Tensor, S: int):
    label = tf.zeros((S, S, 6), tf.float32)
...
DATA_SIZE = {
    'train': 117264,  # 15:22 pre iter
    'val': 4954,  # 33 sec pre iter
}
class COCOBuilder():
...
if __name__ == '__main__':
    ...
    parser.add_argument("--split-size", type=int, default=13)
    ...
    S = args.split_size
    count = 0
    for image, label in tqdm(ds, total=ds_size):
        count += 1
        continue
