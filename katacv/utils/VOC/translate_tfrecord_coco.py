    ...
    def translate(self):
        ...
        with open(str(self.path_idxs)) as csvfile:
            ...
            for image_name, label_name in tqdm(list(reader)):
                try:
                    path_image = self.path_images.joinpath(image_name)
                    example = create_example(path_image, self.path_labels.joinpath(label_name))
                except:
                    print("Wrong Path:", path_image)
...
def parse_args():
    ...
    parser.add_argument("--path-dataset", type=cvt2Path, default=Path("/home/wty/Coding/datasets/COCO/"),
        help="the path of the COCO dataset")
    ...
    args.path_tfrecord = args.path_tfrecord.joinpath(f"COCO-{args.subset}.tfrecord")
    return args
